...
CLocalizer::CLocalizer()
{
    ...
    {
        con_error( "Couldn't find language %s, falling back to %s", m_currentLanguage.c_str(), m_fallBackLanguage.c_str() );
...
void CLocalizer::LoadLangaugeInto(std::string lang, std::map<std::string, std::string>& langMap)
{
    ...
    int64_t fl = 0;
    bool bSuccess = false;
    const uchar_t *languageTomlFile = fileSystem::LoadFile( ("/assets/lang/" + m_languages[lang].langFile).c_str(), fl, bSuccess );
