...
CGameWindow::CGameWindow( const char *title, Vector3i size, bool resizeable, Vector3i minSize )
...
{
	con_warning( "set icon not implemented" );
}
...
Vector3i CGameWindow::GetSize()
{
	int x, y;
	SDL_GetWindowSize( m_pInternalWindow.get(), &x, &y );
	return Vector3i( x, y );
}
void CGameWindow::SetSize( Vector3i size ) { SDL_SetWindowSize( m_pInternalWindow.get(), size.x, size.y ); }

Vector3i CGameWindow::GetPos()
{
	int x, y;
	SDL_GetWindowPosition( m_pInternalWindow.get(), &x, &y );
	return Vector3i( x, y );
}
void CGameWindow::SetPos( Vector3i pos ) { SDL_SetWindowPosition( m_pInternalWindow.get(), pos.x, pos.y ); }
...
void CGameWindow::PollEvents()
{
	...
	m_pInputMan->m_vMouseMovement = Vector3i( 0, 0 );
		...
		switch ( currentEvent.type )
		{
			...
			case SDL_MOUSEMOTION:
				...
				m_pInputMan->m_vMouseMovement =
					m_pInputMan->m_vMouseMovement + Vector3i( currentEvent.motion.xrel, currentEvent.motion.yrel );
				m_pInputMan->m_vMousePos = Vector3i( currentEvent.motion.x, currentEvent.motion.y );
				break;
