...
namespace utfz
{
	...
	cp::cp( const char *str, size_t len ) : Str( str )
	{
		if ( len == -1 )
			End = nullptr;
		else
	...
	cp::cp( const std::string &s ) : Str( s.c_str() ), End( s.c_str() + s.length() ) {}

	///////////////////////////////////////////////////////////////////////////////////////////////

	cp::iter::iter( const char *s, const char *end ) : S( s ), End( end )
	{
	...
	void cp::iter::increment()
	{
		...
		else
		{
			...
			{
				S += len;
				if ( ( len == 2 && ( S[-1] == 0 ) ) || ( len == 3 && ( S[-2] == 0 || S[-1] == 0 ) ) ||
					 ( len == 4 && ( S[-3] == 0 || S[-2] == 0 || S[-1] == 0 ) ) || ( S[0] == 0 ) )
				{
					S = End;
				}
			}
	...
	bool cp::iter::known_end() const { return End != nullptr; }
} // namespace utfz
