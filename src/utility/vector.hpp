...
class Vector3 {
	public:
		...
		Vector3<T> LookingAt(const Vector3<T>& v) const {
			return Vector3<T>(v.x - x, v.y - y, v.z - z);
		}
		Vector3<T> RotateTowards(const Vector3<T>& v) const {
			return Rotate(LookingAt(v), acos(Dot(v)));
		}
...
template <typename T>
Vector3<T> RandomVector3(T min, T max) {
	return Vector3<T>(rand() * (max - min) / RAND_MAX + min, rand() * (max - min) / RAND_MAX + min, rand() * (max - min) / RAND_MAX + min);
}

template <typename T>
Vector3<T> RandomVector3(Vector3<T> min, Vector3<T> max) {
	return Vector3<T>(rand() * (max.x - min.x) / RAND_MAX + min.x, rand() * (max.y - min.y) / RAND_MAX + min.y, rand() * (max.z - min.z) / RAND_MAX + min.z);
}
...
template <typename T>
Vector4<T> RandomVector4(T min, T max) {
	return Vector4<T>(rand() * (max - min) / RAND_MAX + min, rand() * (max - min) / RAND_MAX + min, rand() * (max - min) / RAND_MAX + min, rand() * (max - min) / RAND_MAX + min);
}

template <typename T>
Vector4<T> RandomVector4(Vector4<T> min, Vector4<T> max) {
	return Vector4<T>(rand() * (max.x - min.x) / RAND_MAX + min.x, rand() * (max.y - min.y) / RAND_MAX + min.y, rand() * (max.z - min.z) / RAND_MAX + min.z, rand() * (max.w - min.w) / RAND_MAX + min.w);
}
